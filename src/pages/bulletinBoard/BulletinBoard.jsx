...
export const BulletinBoard = () => {
          ...
          <>
            {authEmployeeState.data.role === '主管' && (
              <Button
                ...
                $bg={'transparent'}
                onClick={() => {
                  navigator(`/bulletin/edit/${params.row._id}`);
                }}
              >
                <Edit /> 編輯
              </Button>
  ...
  return (
    <Layout.PageLayout>
      <SEO title='內部公告 | 漾活管理後台' description={null} url={null} />
      <Container>
        {authEmployeeState.data.role === '主管' && (
          <TitleContainer>
            <h1>內部公告</h1>
        ...
        />
        {/* <pre>{JSON.stringify(authEmployeeState.data, null, 2)}</pre> */}
      </Container>
    </Layout.PageLayout>
  );
};
